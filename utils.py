...
def draw_ui(frame, x1, y1, x2, y2, color, label):
    ...
    cv2.putText(frame, label, (x1, y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
...
def draw_metrics(frame, fps, inf_time):
    ...
    # 3. Add CLEANER metrics text (shorter, larger font)
    metrics_text = f"FPS: {int(fps)}  |  LATENCY: {inf_time:.1f}ms"
    cv2.putText(frame, metrics_text, (20, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
